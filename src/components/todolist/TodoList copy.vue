...
onMounted(async () => {
  try {
    ...
    tasks.value = data.data.map((item: any, index: number) => ({
      id: `task-${index}`,
...
//   const index = selectedDevelopers.value.indexOf(dev);
//   if (index > -1) {
...
// const toggleDeveloper = (dev: string) => {
//   if (selectedDevelopers.value.includes(dev)) {
//     selectedDevelopers.value = selectedDevelopers.value.filter(
//       (d) => d !== dev
//     );
//   } else {
//     selectedDevelopers.value = [...selectedDevelopers.value, dev];
//   }
//   console.log("Selected devs:", selectedDevelopers.value); // Debug
// };
